...
export function create_login_page(document, restIp) {
    ...
    if (container) {
        ...
        elements.submitBtn.addEventListener("click", () => sign_in(container, elements, restIp));
        elements.regBtn.addEventListener("click", () => register(elements, restIp));
    }
}
async function sign_in(container, elements, restIp) {
    let message = {
        userName: elements.userInput.value,
        password: elements.passInput.value,
    };
    let response = await (await fetch(restIp + "" + "/sign_in", {
        method: "POST",
...
async function register(elements, restIp) {
    ...
    let response = await (await fetch(restIp + "" + "/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(message),
    })).json();
